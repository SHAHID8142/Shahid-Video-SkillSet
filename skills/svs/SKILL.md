...
2. **Detect the stack.** Next.js (App/Pages Router) · Vite + React · CRA · Astro · plain HTML/CSS.
   **Non-React Fallback (Vue / Svelte / Angular):** Remotion is React-based. If the target project uses a non-React framework, you MUST use Playwright/Puppeteer to capture high-fidelity screenshots/video of the actual running app (on `localhost`) and composite them in Remotion instead of component-level alias imports. Read `package.json`, framework config, and the Tailwind/PostCSS/global CSS setup.
...
- **Complex State Mocking:** If a component relies on global providers (Router, Redux, Zustand, ThemeProvider), you MUST create an `IsolatedProvider.tsx` wrapper in `video/src/` that supplies these mock contexts. Do not edit the user's component to remove the context dependencies.
...
## STEP 4.6 — 🛠️ ERROR RECOVERY & AUTO-FIXING

When the Remotion preview or build throws errors, you must recover systematically:
- **Webpack / Vite Alias Failures**: If imports fail, explicitly check the module resolution paths. Ensure `remotion.config.ts` has the correct absolute `path.join(__dirname, '../src')` overrides and that PostCSS/Tailwind plugins are correctly mapped.
- **Missing Contexts / Hooks**: If a component crashes complaining about `useRouter`, `useStore`, or a missing Provider, do not remove the hook. Wrap the component invocation inside your scene with an isolated mock provider.
- **Broken Renders / Blank Screens**: Typically caused by non-deterministic async data. Ensure `delayRender` and `continueRender` are strictly wrapping any asynchronous font loading, image fetching, or data mocking.
...
1. **Composition shell.** Create the root `<Composition>` and a `Root.tsx` registering it. Set fps and
   ...
   - **TTS & Narration**: If voiceover is requested, use a TTS service (like ElevenLabs) or local Node TTS scripts to generate the `.mp3` narration track. Place it in `public/` and use `<Audio>` with volume ducking (lower the music bed volume dynamically when narration is speaking).
...
2. Full render at target quality. Choose the correct export pipeline based on the platform:
   - **YouTube / Standard HD (H.264)**:
     ```bash
     npx remotion render Video out/showcase.mp4 --codec=h264 --crf=18 --scale=1 --gl=angle
     ```
   - **Transparent Overlays / Alpha Channel**:
     ```bash
     npx remotion render Video out/showcase.webm --codec=webm --transparent
     # OR for Mac/Pro video editors:
     npx remotion render Video out/showcase.mov --codec=prores --prores-profile=4444
     ```
   - **GPU-Safe Headless Rendering**: If the render crashes on WebGL/GPU steps in CI or headless environments, switch the GL backend: `--gl=swangle` (software, most compatible for cloud/CI) or `--gl=angle` (GPU, best for local Three.js/WebGL). Valid backends: `swangle`, `angle`, `egl`, `swiftshader`, `vulkan`, `angle-egl`.
   For Reels/TikTok, ensure you are rendering the specific 9:16 vertical composition.
